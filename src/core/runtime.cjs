...
function createRuntime(options = {}) {
	...
	function readSnapshotForProject(project, create = true) {
		...
		const cards = readCards(store.rootPath);
	...
	function activeSnapshot() {
		...
		return stripInternalSnapshotFields(readSnapshotForProject(project, true));
	}

	function activeSnapshotWithInternals() {
		const registry = readRegistry();
		const project = activeProjectFromRegistry(registry);
		if (!project || !fs.existsSync(project.path)) return null;
	...
	function chooseProjectPath(projectPath) {
		const canonicalPath = fs.realpathSync(projectPath);
		const registry = readRegistry();
		const existing = registry.projects.find((project) => project.path === canonicalPath);
		if (existing) {
			registry.activeProjectId = existing.id;
			writeRegistry(registry);
			return stripInternalSnapshotFields(readSnapshotForProject(existing, true));
		}
	...
	function locateProjectPath(projectId, projectPath) {
		const canonicalPath = fs.realpathSync(projectPath);
		const registry = readRegistry();
		const project = registry.projects.find((entry) => entry.id === projectId);
	...
	function setStorageSearchPaths(paths) {
		const registry = readRegistry();
	...
	function setActiveWorkspaceFile(filePath) {
		const registry = readRegistry();
...
function stripInternalSnapshotFields(snapshot) {
...
function activeProjectFromRegistryShape(registry) {
	const id = registry.activeProjectId;
	if (!id) return null;
	const manual = registry.projects.find((project) => project.id === id);
	if (manual) return manual;
...
function listWorktrees(registry) {
	const activeProject = activeProjectFromRegistryShape(registry);
